...
# MinIO -> ClickHouse -> RBAC multi-tenant -> tabelas de fato.
...
echo "==> 1/8 cluster local"
bash cluster/minikube-up.sh

echo "==> 2/8 build da imagem Spark (dentro do minikube)"
...
echo "==> 5/8 Spark Operator + RBAC/secrets"
...
echo "==> 6/8 ClickHouse operator + instância"
...
echo "==> 7/8 RBAC multi-tenant (database/role/user/profile/quota por tenant)"
...
echo "==> 8/8 tabela de fato por tenant"
